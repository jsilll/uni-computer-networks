...
int unregisterUser(char *uid, char *pass)
{
  ...
  unlink(PATH_BUFFER); /* deletes login file */

  sprintf(PATH_BUFFER, "USERS/%s", uid);
  rmdir(PATH_BUFFER); /* deletes user directory */

  /* deletes user from every group it was subscribed to */
  for (int i = 0; i < 100; i++)
  {
...
int logout(char *uid, char *password)
{
  ...
  {
    return -1; /* User not logged in - NOK */
  }

  sprintf(PATH_BUFFER, "USERS/%s/password.txt", uid);
  if (checkFileContent(PATH_BUFFER, password) == -1)
  {
    return -1; /* Incorrect password - NOK */
  }
...
int subscribe(char *uid, char *gid, char *gname)
{
  ...
  {
    return -1; /* Group doesn't exist or user not logged in - NOK */
  }
  ...
  if (!strcmp(gid, "00"))
  {
    if (N_GROUPS >= 99)
    {
      return -2; /* Max groups reached - E_FULL */
    }
    else
    {
      N_GROUPS++;
      ...
      return N_GROUPS; /* retrurn new GID */
    }
  }
  ...
  if (checkFileContent(PATH_BUFFER, gname) == -1)
    return -1; /* Wrong group name - NOK */

  sprintf(PATH_BUFFER, "GROUPS/%s/%s.txt", gid, uid);
  createFile(PATH_BUFFER, "");
  return 0; /* Subscribed - OK */
}
...
void listGroups(char *buffer, char *uid)
{
  ...
  if ((uid != NULL) && userLoggedIn(uid) == -1)
  {
    strcpy(buffer, "RGM E_USR\n"); /* User not logged in (my_groups only) - NOK */
    return;
  }
...
DIR *ulist(char *gid)
{
  ...
  {
    return NULL; /* Group doesn't exist - NOK */
  }
...
/**
 * @brief Gets user subscribed to a group
 * appends it with a space: " "
 * @param de
 * @param buffer
 */
...
FILE *post(char *uid, char *gid, char *text, char *fname, char *mid)
{
  ...
  {
    return NULL; /* Group doesn't exist or user not logged in - NOK */
  } 
  ...
  if ((fptr = fopen(PATH_BUFFER, "r")) == NULL)
  {
    ...
    return NULL; /* User not subscribed to group - NOK */
  }
  fclose(fptr);

  /* increments number of messages on a group */
  ...
  /* Creates message directory */
  sprintf(PATH_BUFFER, "GROUPS/%s/MSG/%s", gid, mid);
  mkdir(PATH_BUFFER, 0700);

  /* Creates message author and text files */
  ...
  /* Creates filename file and opens file pointer to write the data */
  if (fname != NULL)
  {
...
/**
 * @brief Check how many messages to be retrieved (maximum 20)
 *
 * @param uid
 * @param gid
 * @param mid
...
int getNumMsg(char *uid, char *gid, char *mid)
{
  ...
  if ((fptr = fopen(PATH_BUFFER, "r")) == NULL)
  {
    return -1; /* User not subscribed to group - NOK */
  }
  fclose(fptr);
  ...
  {
    return -1; /* Message id beyond existing messages - NOK */
  }
...
/**
 * @brief Gets the text, author, filename and file pointer if data file exists
 * Writes information to buffer directly
 *
 * @param gid
 * @param mid
 * @param buffer
